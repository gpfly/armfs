...
static int mill_fasync (int fd, struct file *filp, int mode)
{
...
static void do_mill_timer(unsigned long data)
{
	...
	kill_fasync(&fasync_queue, SIGIO, POLL_IN); //给对应进程发信号
...
static int __init demo_init(void)
{
	...
	tasklet_init(&task, do_bh_handler, 0);
	setup_timer(&timer, do_mill_timer, 0);
