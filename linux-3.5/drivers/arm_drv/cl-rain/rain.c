...
mill_read (struct file *filp, char __user *buf, size_t cnt, loff_t *fpos)
{
	...
	result = s3c_adc_read(myres2.client, myres2.channal);
	if (result < 0) {
		return -EINVAL;
	}
...
static int res_probe (struct platform_device *pdev)
{
	...
	myres2.client = s3c_adc_register(pdev, NULL, NULL, 0);
	if (IS_ERR(myres2.client)) {
		return -EINVAL;
	}

	mutex_init(&myres2.mutex);
