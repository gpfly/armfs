...
static int mill_probe(struct platform_device *pdev)
{
	...
	struct resource *src = platform_get_resource_byname(pdev, IORESOURCE_MEM, "changbeep");

	int gpionum = mybeep.gpionum = src->start;	

	if (gpio_request(gpionum, "changbeep") < 0) {
	...
	mybeep.pwm = pwm_request(mybeep.id_num, "changbeep");
