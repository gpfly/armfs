...
mill_read (struct file *filp, char __user *buf, size_t cnt, loff_t *fpos)
{
	...
	result = s3c_adc_read(myres1.client, myres1.channal);
	if (result < 0) {
		return -EINVAL;
	}
...
static int res_probe (struct platform_device *pdev)
{
	...
	myres1.client = s3c_adc_register(pdev, NULL, NULL, 0);
	if (IS_ERR(myres1.client)) {
		return -EINVAL;
	}

	mutex_init(&myres1.mutex);
