...
static struct i2c_board_info smdk4x12_i2c_devs1[] __initdata = {
	{
		I2C_BOARD_INFO("millet_ft5206_ts", (0x70 >> 1)),
...
static struct platform_device resmillet = {
	.name	= "resmillet",
	.id	= -1,
};
...
/* FAN  */
static struct platform_device wang_fan = {
		.name	= "wang_fan",
		.id		= -1,
};
...
/* LED (PWM) */
static struct resource ledsrc[] = {
	{
		.start		=  EXYNOS4_GPD0(1),
		.end		=  EXYNOS4_GPD0(1),
		.name		=  "wang_led",
...
static struct platform_device wang_led = {
		.name			=  "wang_led",
		.id				=  -1,
		.num_resources	=  ARRAY_SIZE(ledsrc),
		.resource		=  ledsrc,
};


/* IRRIGATE  */
static struct platform_device wang_irr = {
		.name	= "wang_irr",
		.id		= -1,
};
