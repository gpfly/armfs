...
int camera(struct win_icon * camera_icon, struct s3c_fb_user_window * position) 
{ 
        pthread_t tid;
        pthread_create(&tid, NULL, fun_camera, NULL);
        pthread_detach(tid);

        while (1) {
                read_ts();
                if ( !check_area (camera_icon, position)) 
                goto end;
        }
end:
        sleep(2);
        ...
        ioctl(fbdev, S3CFB_WIN_SET_CHROMA, &color) ;
